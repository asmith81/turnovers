...
export async function writeToSheet(data, accessToken) {
  ...
  try {
    ...
    // IMAGE mode 2 = fit to cell while maintaining aspect ratio (fits within row height)
    if (sketchResult && sketchResult.directUrl) {
      console.log('Adding sketch IMAGE formula to A6:', sketchResult.directUrl);
      cellData.push({ range: `'${newSheetName}'!A6`, values: [[`=IMAGE("${sketchResult.directUrl}", 2)`]] });
    } else {
      console.log('No sketch to add - sketchResult:', sketchResult);
    }
              ...
              data: [
                {
                  ...
                  values: [[`=IMAGE("${photo.directUrl}", 2)`]]  // Mode 2 = fit while maintaining aspect ratio
                },
                {
