...
export async function processWithLLM(userInput, currentData, conversationHistory) {
  // Define the expected structure matching the turnover template
  const dataSchema = {
    workOrderNumber: '',
    unitNumber: '',
    address: '',
    ...
    workItems: [
      ...
      // {
      //   category: 'Painting | Floor & Molding | Doors & Windows | Other | ...',
      //   item: 'Clean Walls | Patch Hole | ...',
      //   description: 'Clean | Paint | Install | Remove & Install | ...',
      //   unit: 'SF | LF | EA | SET',
      //   multiplier: number (quantity),
      //   pricePerUnit: number (from catalog),
      //   total: number (multiplier * pricePerUnit),
      //   notes: 'Spanish or English notes'
      // }
    ]
  };
...
Work items must match the pricing catalog structure:
- category: Must be one of: "Painting", "Floor & Molding", "Doors & Windows", "Other", "Outlets No Wiring", "Light Switch No Wiring", "Electrical Installation", "Plumbing Installation", "Plumbing Repairs", "Clean Up"
- item: Must match items available in that category (e.g., "Clean Walls", "Prep & Paint Walls 2 Coats")
- description: Type of work: "Clean", "Paint", "Install", "Remove & Install", "Demolition", "Repair", etc.
- unit: "SF" (square foot), "LF" (linear foot), "EA" (each), or "SET" (set)
- multiplier: quantity of units needed (numeric)
- pricePerUnit: automatically looked up from pricing catalog based on category + item + description
- total: calculated as multiplier * pricePerUnit
- notes: Additional details in Spanish or English

When user describes work in natural language:
1. Map to correct category (e.g., "paint walls" → "Painting")
2. Select appropriate item (e.g., "paint 3 walls" → "Prep & Paint Walls 2 Coats")
3. Determine unit and quantity (e.g., "244 square feet" → multiplier: 244, unit: SF)
4. Look up price from catalog automatically
5. Ask for room/location details to add in notes
