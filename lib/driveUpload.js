...
export async function uploadSketchToDrive(sketchDataUrl, fileName, accessToken) {
  ...
  try {
    ...
    const uploadWithTimeout = async () => {
      ...
      // Use parent folder from env if specified, otherwise create in user's Drive
      const parentFolderId = process.env.GOOGLE_DRIVE_FOLDER_ID || null;
      
      // Find or create the Turnovers_Sketches folder
      const folderId = await findOrCreateFolder(drive, 'Turnovers_Sketches', parentFolderId);
      console.log('Sketch folder ID:', folderId, parentFolderId ? '(in parent folder)' : '(in Drive root)');
      ...
      console.log('Sketch uploaded with file ID:', fileId);
      ...
      console.log('Sketch permissions set to public');
    ...
    // Use the lh3 URL format which is more reliable for Google Sheets IMAGE function
    // The uc?export=view format has become unreliable
    const directUrl = `https://lh3.googleusercontent.com/d/${fileId}`;
    console.log('Sketch direct URL for Sheets:', directUrl);
...
export async function uploadPhotosToDrive(photos, workOrderNumber, accessToken, onProgress = null) {
  ...
  try {
    ...
    const parentFolderId = await findOrCreateFolder(drive, 'Turnovers_Photos', rootFolderId);
    console.log('Photos parent folder ID:', parentFolderId, rootFolderId ? '(in configured folder)' : '(in Drive root)');
    ...
    for (let i = 0; i < photos.length; i++) {
      ...
      try {
        ...
        const fileId = await Promise.race([uploadWithTimeout(), timeoutPromise]);
        // Use lh3 URL format which is more reliable for Google Sheets IMAGE function
        const directUrl = `https://lh3.googleusercontent.com/d/${fileId}`;
